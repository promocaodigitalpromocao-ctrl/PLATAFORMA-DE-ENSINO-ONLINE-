...
import { X, UploadCloud, FileVideo, Loader2, Play, ShieldCheck, ArrowRightCircle, PlusSquare, HardDrive } from 'lucide-react';
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      if (!file.type.includes('mp4') && !file.type.includes('webm')) {
          alert("Por favor, envie apenas arquivos MP4 ou WebM.");
          return;
      }
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-2xl w-full max-w-4xl h-[80vh] flex flex-col shadow-2xl animate-in fade-in zoom-in-95 duration-200 relative">
        ...
        <div className="flex justify-between items-center p-6 border-b border-zinc-800 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-white">Biblioteca de Testes</h2>
           ...
           >
             ...
             {isUploading ? (
               <div className="flex flex-col items-center">
                 ...
                 <span className="text-zinc-300 font-medium">Processando arquivo... {uploadProgress}%</span>
               </div>
             ) : (
               <>
                 ...
                 <h3 className="text-lg font-medium text-white mb-1">Carregar Vídeo de Teste</h3>
                 <p className="text-zinc-500 text-sm font-medium">Os arquivos ficam salvos localmente para pré-visualização</p>
               </>
             )}
           </button>
        </div>
              ...
              <div key={video.id} className="bg-zinc-950 border border-zinc-800 rounded-xl p-4 flex items-center justify-between hover:border-zinc-700 transition-colors">
                ...
                <div className="flex items-center gap-2 shrink-0 ml-4">
                  {onSelect && (
                      ...
                      >
                         SELECIONAR <ArrowRightCircle size={16} />
                      </button>
                  )}
              ...
              {previewVideoId && videos.find(v => v.id === previewVideoId && !v.approved) && (
                  <div className="p-4 bg-emerald-500/10 border-t border-emerald-500/20 flex items-center justify-between">
                      <div className="flex items-center gap-2 text-emerald-500">
                          <ShieldCheck size={20} />
                          <span className="font-medium">Confirme se o vídeo está carregando corretamente.</span>
                      </div>
                      <button 
                        onClick={async () => {
                            if (previewVideoId) {
